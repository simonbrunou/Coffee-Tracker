...
- **Login/signup forms now do a DB lookup for authenticated-but-stale sessions.** Switching the `/login` and `/signup` redirect guards from raw `auth()` to revocation-aware `getCurrentUserId` means that, during a DB outage, an authenticated visitor with a stale cookie hits the error boundary instead of the form. Anonymous visitors are unaffected (zero-DB short-circuit). Accepted: consistent with the rest of the app requiring the DB, and it's the fix that lets a revoked user reach the form to re-authenticate.
