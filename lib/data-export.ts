...
/** Gather EVERYTHING the caller owns for a self-service data export (DSAR /
 *  right-to-access). Scoped entirely to `userId` — no other user's private data.
 *  Excludes secrets (password_hash, session_version, OAuth tokens). */
export async function getDataExport(userId: string): Promise<Record<string, unknown>> {
  const [profile, brews, bags, saved, wishlist, comments, followUsers, followRoasters, providers] = await Promise.all([
    ...
    getMyTastings(userId),
    getMyShelf(userId),
    getSavedTastings(userId),
    getWishlistBeans(userId),
